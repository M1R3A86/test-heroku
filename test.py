...
class Bot:
		...
		if message_id == None:
			...
			while t == False:
				try:
					...
					}))},url="https://shadmessenger73.iranlms.ir/")
				...
				if message_id == None:
					...
					while t == False:
						try:
							...
							}))},url="https://shadmessenger73.iranlms.ir/").text)['data_enc']))
							t = True
						except:
							t = False
					return p
	...
	def getUserInfo(self, chat_id):
		...
		}))},url="https://shadmessenger73.iranlms.ir/").json()["data_enc"]))
	...
	def getMessages(self, chat_id,min_id):
		...
		}))},url="https://shadmessenger73.iranlms.ir/").json().get("data_enc"))).get("data").get("messages")
	...
	def getInfoByUsername(self, username):
		...
		}))},url="https://shadmessenger73.iranlms.ir/").json().get("data_enc")))
	...
	def getMessagesInfo(self, chat_id, message_ids):
		...
		}))}, url="https://shadmessenger73.iranlms.ir/").json()["data_enc"])).get("data").get("messages")
	...
	def get_updates_all_chats(self):
		...
		}))},url="https://shadmessenger73.iranlms.ir/").json().get("data_enc"))).get("data").get("chats")
	...
	def get_updates_chat(self, chat_id):
		...
		}))},url="https://shadmessenger73.iranlms.ir/").json().get("data_enc"))).get("data").get("updated_messages")
	...
	def my_sticker_set(self):
		...
		}))},url="https://shadmessenger73.iranlms.ir/").json().get("data_enc"))).get("data")
...
auth = "chbfjgnchftcpxiumyufnmdjdealbgbi"
bot = Bot(auth)
list_message_seened = []
